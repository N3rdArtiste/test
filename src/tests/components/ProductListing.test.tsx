...
test('renders ProductListing correctly', () => {
  ...
  const {getByText,getAllByText} = render(<ProductListing onAddToCartClick={mockCallBack} data={productsMock} />);
...
it('should execute callback on clicking "add to cart" button', () => {
    const mockCallBack = jest.fn()
  const {getAllByText} = render(<ProductListing onAddToCartClick={mockCallBack} data={productsMock} />);
